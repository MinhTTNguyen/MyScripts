...
my $path="/home/mnguyen/Research/Lysozyme/Non_microorganisms/Download_CAZy_eukaryota";
my $filein_cazy_list="CAZy_list.txt";
my $folderout="HTML_Eukaryota";
mkdir "$path/$folderout";
#========================================================================================================#
# Get list of CAZy families
my @cazy_list;
open(List,"<$path/$filein_cazy_list") || die "Cannot open file $filein_cazy_list";
while (<List>)
{
	$_=~s/\s*//g;
	push(@cazy_list,$_);
...
foreach my $cazy_family (@cazy_list)
{
	...
	my $url_all='http://www.cazy.org/'.$cazy_family.'_eukaryota.html';
	...
	while (<First_table>)
	{
		...
		if ($_=~/href\=\'$cazy_family\_eukaryota\.html\?debut\_TAXO\=(\d+)\#pagination\_TAXO\'/)
		{
			my $temp=$1;
	...
	if ($number)
	{
		for (my $i=100; $i<=$number; $i=$i+100)
		{
			...
			my $url='http://www.cazy.org/'.$cazy_family.'_eukaryota.html?debut_TAXO='.$i.'#pagination_TAXO';
