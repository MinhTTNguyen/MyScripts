# October 18th 2017
# Get the best domain pfam module
...
my $filein="/home/mnguyen/Research/A_nidulans/Workshop_09Aug2018/All_PFAM_from_InterProScan_Anidulans_09Aug2018_1prot_1row_all_domains.txt";
my $fileout="/home/mnguyen/Research/A_nidulans/Workshop_09Aug2018/All_PFAM_from_InterProScan_Anidulans_09Aug2018_1prot_1row_best_domains.txt";


open(In,"<$filein") || die "Cannot open file $filein";
open(Out,">$fileout") || die "Cannot open file $fileout";
print Out "#Seq_id\tpfam_module\tpfam_module (evalue, HMM coverage, domain_start, domain_end)\n";
while (<In>)
{
	if($_!~/^\#/)
	{
		...
		my $id=$cols[0];
		my $pfam=$cols[1];
		my @regions=split(/ \- /,$pfam);
		my $best_pfam_module_short="";
		my $best_pfam_module_long="";
		foreach my $region (@regions)
		{
			my $selected_pfam_long=&Process_overlap_region($region);
			if ($best_pfam_module_long){$best_pfam_module_long=$best_pfam_module_long." - ".$selected_pfam_long;}
			else{$best_pfam_module_long=$selected_pfam_long;}
		}
		my @families_long=split(/ \- /,$best_pfam_module_long);
		foreach my $family_long (@families_long)
		{
			$family_long=~s/\s*\(.+\)//;
			if ($best_pfam_module_short){$best_pfam_module_short=$best_pfam_module_short." - ".$family_long;}
			else{$best_pfam_module_short=$family_long;}
		}
		print Out "$id\t$best_pfam_module_short\t$best_pfam_module_long\n";
	}
}
close(In);
close(Out);
